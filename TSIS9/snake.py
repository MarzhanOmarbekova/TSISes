    ...
    while cant_choose:
        ...
        for i in snake_body:
            if i==food_pos:
                cant_choose = True
                break
    pg.draw.rect(screen, food_color, (food_pos[0], food_pos[1], 20, 20))


DIFF_WEIGHT = pg.USEREVENT +1
pg.time.set_timer(DIFF_WEIGHT, 14001)
DIFF_WEIGHT_TIME = pg.USEREVENT +2
pg.time.set_timer(DIFF_WEIGHT_TIME, 7000)
...
while not done:
    for event in pg.event.get():
        ...
        if event.type == DIFF_WEIGHT:
            food_color = 'blue'
            food_to_screen(food_color)
        if event.type == DIFF_WEIGHT_TIME :
            food_color="white"
            food_to_screen(food_color)
